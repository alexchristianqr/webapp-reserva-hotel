...
Esquema real de la base de datos `db_hotel` (ver `db_hotel.sql`) y su correspondencia
con los POJOs de `src/java/models/`. Esta es la **fuente de verdad** del modelo de datos.
...
| pwd | VARCHAR(250) | ⚠️ texto plano hoy |
...
| `Prueba` | — | modelo de experimentación/pruebas |
...
- Al crear una persona-derivada (cliente/empleado) se inserta primero en `personas`
  y luego en la tabla específica usando el `id` generado.
